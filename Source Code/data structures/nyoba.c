...
void insertNode(struct MahasiswaNode **tree, const double ipk), tampilkanNode(struct MahasiswaNode *tree);
...
void tampilkanNode(struct MahasiswaNode *tree)
{
    if (tree != NULL)
    {
        /* left */
        tampilkanNode(tree->leftChild);

        /* root */
        printf("%.2f\n", tree->ipk);

        /* right */
        tampilkanNode(tree->rightChild);
    }
}
